...
public class CommentDAO extends DBConnection {
    ...
    public List<Comment> getCommentOfPost(int postId) {
        ...
        try {
            String SQL = "SELECT * FROM Comment WHERE postId=? AND responeTo IS NULL AND [status]=?";
            connection = getConnection();
            pstm = connection.prepareStatement(SQL);
            pstm.setInt(1, postId);
            pstm.setInt(2, Comment.STATUS_ACTIVED);
    ...
    public List<Comment> getResponseComment(int cmtId) {
        List<Comment> result = null;
        try {
            String SQL = "SELECT * FROM Comment WHERE responeTo=? AND [status]=?";
            connection = getConnection();
            pstm = connection.prepareStatement(SQL);
            pstm.setInt(1, cmtId);
            pstm.setInt(2, Comment.STATUS_ACTIVED);
